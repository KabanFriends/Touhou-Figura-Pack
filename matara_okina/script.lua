...
model.all.helmet.setEnabled(false)
...
network.registerPing("enableDoors")
network.registerPing("disableDoors")

function enableDoors() model.all.helmet.setEnabled(true) end
function disableDoors() model.all.helmet.setEnabled(false) end

--action wheel
item = item_stack.createItem("minecraft:iron_door")
action_wheel.SLOT_1.setItem(item)
action_wheel.SLOT_1.setTitle("Enable Doors")
action_wheel.SLOT_1.setFunction(function()
	if (model.all.helmet.getEnabled() == false) then
		network.ping("enableDoors")
		item = item_stack.createItem("minecraft:iron_door")
		action_wheel.SLOT_1.setItem(item)
		action_wheel.SLOT_1.setTitle("Disable Doors")
		model.all.helmet.setEnabled(true)
	else
		network.ping("disableDoors")
		item = item_stack.createItem("minecraft:iron_door")
		action_wheel.SLOT_1.setItem(item)
		action_wheel.SLOT_1.setTitle("Enable Doors")
		model.all.helmet.setEnabled(false)
